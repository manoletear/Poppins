          ...
          <div className="w-full max-w-3xl rounded-2xl bg-white shadow-2xl overflow-hidden">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {plans.map((plan) => (
                  <div key={plan.id} className={`relative rounded-xl border-2 p-5 transition-all ${plan.current ? 'border-blue-500 bg-blue-50/50 shadow-md' : 'border-zinc-200 hover:border-zinc-300'}`}>
                    {plan.current && (
                    ...
                    <div className="mt-4">
                      {plan.current ? (
                        <div className="w-full rounded-lg bg-blue-100 py-2 text-center text-sm font-medium text-blue-700">Plan Actual</div>
                      ) : plan.id === 'enterprise' ? (
                        <button className="w-full rounded-lg bg-zinc-900 py-2 text-sm font-medium text-white hover:bg-zinc-800 transition-colors">Hacer Upgrade</button>
                      ) : (
                        <button className="w-full rounded-lg border border-zinc-300 py-2 text-sm font-medium text-zinc-600 hover:bg-zinc-50 transition-colors" disabled>Downgrade</button>
                      )}
                    </div>
                  </div>
                ))}
              </div>
              <p className="text-xs text-zinc-400 text-center mt-4">Los cambios de plan se aplican en el siguiente ciclo de facturación.</p>
            </div>
          </div>
        </div>
      )}
