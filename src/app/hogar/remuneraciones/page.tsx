...
const PROCESOS_CERRADO = [
  ...
  { key: 'contabilidad',      label: 'Contabilidad',         Icon: FileSpreadsheet, download: 'contabilidad'    },
  { key: 'previred',          label: 'Previred',             Icon: Shield,          download: 'previred'        },
  { key: 'libro_electronico', label: 'Libro Electrónico',    Icon: Receipt,         download: 'libro_electronico' },
];
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (documento) {
        ...
        fd.set('trabajador_id', form.trabajador_id);
        fd.set('periodo', period);
        fd.set('tipo', form.tipo);
        fd.set('fecha_inicio', form.fecha_inicio);
        fd.set('fecha_fin', form.fecha_fin);
        fd.set('observacion', form.observacion);
        fd.set('documento', documento);
        res = await fetch('/api/payroll/licencias-medicas', { method: 'POST', body: fd });
      } else {
        res = await fetch('/api/payroll/licencias-medicas', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
              ...
              <>
                    ...
                    {licencias.map(lic => (
                      <div key={lic.id} className="flex items-start justify-between rounded-xl border border-zinc-200 px-3 py-2.5 gap-3">
                        <div className="min-w-0">
                          ...
                          {(lic as any).documento_url && (
                            <a href={(lic as any).documento_url} target="_blank" rel="noreferrer"
                              className="inline-flex items-center gap-1 text-[10px] text-[#1a2e6e] hover:underline mt-1">
                              <FileText className="w-3 h-3" />
                              {(lic as any).documento_nombre ?? 'Documento adjunto'}
                            </a>
                          )}
                        </div>
                ...
                <div className="rounded-xl border border-dashed border-zinc-300 p-3 space-y-2.5">
                  ...
                  <input type="text" placeholder="Observación (opcional)" value={form.observacion}
                    ...
                    className="w-full text-xs rounded-lg border border-zinc-300 px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-[#1a2e6e]" />
                  <div>
                    <label className="text-[10px] text-zinc-500 mb-0.5 block">Documento (PDF/imagen, opcional)</label>
                    <input type="file" accept=".pdf,image/*"
                      onChange={e => setDocumento(e.target.files?.[0] ?? null)}
                      className="w-full text-xs text-zinc-600 file:mr-2 file:py-1.5 file:px-2.5 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-zinc-100 file:text-zinc-700 hover:file:bg-zinc-200" />
                  </div>
...
// ── Modal Liquidaciones (período cerrado) ────────────────────────────────────

function LiquidacionesModal({ period, onClose }: { period: string; onClose: () => void }) {
  const [workers, setWorkers] = useState<{ workerId: string; workerName: string; workerRut: string; netPay: number }[]>([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    fetch(`/api/payroll/trabajadores-periodo?period=${period}`)
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (d?.data) setWorkers(d.data); })
      .catch(() => {})
      .finally(() => setLoading(false));
  }, [period]);

  const handleDownloadPdf = (workerId: string, workerRut: string) => {
    const a = document.createElement('a');
    a.href = `/api/payroll/liquidacion-pdf?period=${period}&workerId=${workerId}`;
    a.download = `liquidacion_${period.replace('-', '')}_${workerRut.replace(/\./g, '').replace('-', '')}.pdf`;
    a.click();
  };
  ...
  const handleDownloadZip = async () => {
    ...
    try {
      const a = document.createElement('a');
      a.href = `/api/payroll/liquidaciones-zip?period=${period}`;
      a.download = `liquidaciones_${period.replace('-', '')}.zip`;
      a.click();
    } finally {
  ...
  const handleDownloadMerged = async () => {
    ...
    try {
      const a = document.createElement('a');
      a.href = `/api/payroll/liquidaciones-merged?period=${period}`;
      a.download = `liquidaciones_${period.replace('-', '')}.pdf`;
      a.click();
    } finally {
      setTimeout(() => setZipping(false), 1500);
    }
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 bg-black/40" onClick={onClose} />
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm pointer-events-auto flex flex-col max-h-[70vh]">
          <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-200 shrink-0">
            <div className="flex items-center gap-2">
              <FileText className="w-4 h-4 text-zinc-500" />
              <h2 className="text-sm font-semibold text-zinc-800">Liquidaciones — {periodoLabel(period)}</h2>
            </div>
            <button onClick={onClose} className="text-zinc-400 hover:text-zinc-600 transition"><X className="w-4 h-4" /></button>
          </div>
          <div className="flex-1 overflow-y-auto px-5 py-4">
            {loading ? (
              <div className="flex justify-center py-6"><Loader2 className="w-5 h-5 animate-spin text-zinc-400" /></div>
            ) : workers.length === 0 ? (
              <p className="text-xs text-zinc-400 text-center py-4">Sin resultados para este período.</p>
            ) : (
              <div className="space-y-2">
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    disabled={zipping}
                    className="flex items-center justify-center gap-1.5 text-xs px-3 py-2 rounded-xl bg-[#1a2e6e] text-white hover:bg-[#142358] transition disabled:opacity-50"
                  >
                    {zipping
                      ? <Loader2 className="w-3.5 h-3.5 animate-spin" />
                      : <><Download className="w-3.5 h-3.5" /> ZIP ({workers.length})</>}
                  </button>
                  <button
                  ...
                  >
                    {zipping
                      ? <Loader2 className="w-3.5 h-3.5 animate-spin" />
                      : <><FileText className="w-3.5 h-3.5" /> PDF único</>}
                  </button>
                </div>
                <div className="text-[10px] text-zinc-400 text-center pt-1 pb-1">o una por una</div>
                {workers.map(w => (
                  <div key={w.workerId} className="flex items-center justify-between rounded-xl border border-zinc-200 px-3 py-2.5">
                    <div>
                      <p className="text-xs font-semibold text-zinc-800">{w.workerName}</p>
                      <p className="text-[11px] text-zinc-400">{w.workerRut} · ${fmt(w.netPay)} neto</p>
                    </div>
                    <button
                      onClick={() => handleDownloadPdf(w.workerId, w.workerRut)}
                      className="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg border border-[#1a2e6e] text-[#1a2e6e] hover:bg-blue-50 transition"
                    >
...
export default function RemuneracionesPage() {
  ...
  const [liquidacionesModal, setLiquidacionesModal] = useState<string | null>(null);
  ...
  const handleProcesar = async (period: string, mode: 'preview' | 'final') => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        alert(data.detail ?? data.error ?? 'Error al procesar');
      }
  ...
  const handleReopenPeriod = async (period: string) => {
    ...
    else if (data.error === 'periodos_posteriores_cerrados') {
      alert(data.detail ?? `Hay períodos posteriores cerrados. Reabre primero ${data.nextClosedPeriod}.`);
    } else alert(data.detail ?? data.error ?? 'Error al reabrir período');
  };
  ...
  return (
    <div className="space-y-0 max-w-4xl">
      ...
      {periods.map((period) => {
        ...
        // Secuencialidad: prev period y next period (para bloquear cierre/reapertura)
        const [py, pm] = period.split('-').map(Number);
        const prevD = new Date(py, pm - 2, 1);
        const nextD = new Date(py, pm, 1);
        ...
        const prevSummary = summaries.get(prevPeriod);
        const nextSummary = summaries.get(nextPeriod);
        // Bloquear cierre si prev existe en la lista pero NO está cerrado
        const blockClose  = !!prevSummary && !prevSummary.closed;
        // Bloquear reapertura si next existe y está cerrado
        const blockReopen = !!nextSummary && nextSummary.closed;
              ...
              <div className="px-4 pb-5">
                {isClosed ? (
                      ...
                      <div className="relative">
                        ...
                        {masAcciones === period && (
                          <>
                            ...
                            <div className="absolute right-0 mt-1 w-56 rounded-xl border border-zinc-200 bg-white shadow-xl z-50 overflow-hidden">
                              <button
                                onClick={() => handleReopenPeriod(period)}
                                disabled={blockReopen}
                                title={blockReopen ? `Reabre primero ${periodoLabel(nextPeriod)}` : ''}
                              ...
                              >
                                ...
                                {blockReopen && (
                                  <div className="text-[10px] text-zinc-400 mt-0.5 leading-tight">
                                    Reabre primero {periodoLabel(nextPeriod)}
                                  </div>
                                )}
                              </button>
                            </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {PROCESOS_CERRADO.map(({ key, label, Icon, download }) => {
                        const onClickFn = download === 'previred'
                          ...
                          : download === 'sueldos'
                          ? () => handleDownload(`/api/payroll/sueldos?period=${period}`, `sueldos_${period.replace('-','')}.xlsx`)
                          : download === 'libro'
                          ...
                          ? () => handleDownload(`/api/payroll/contabilidad?period=${period}`, `contabilidad_${period.replace('-','')}.xlsx`)
                          : download === 'libro_electronico'
                          ? () => handleDownload(`/api/payroll/libro-remuneraciones?period=${period}`, `LRE_${period.replace('-','')}.txt`)
                          : download === 'liquidaciones'
                          ? () => setLiquidacionesModal(period)
                          : undefined;
                        return (
                          <button
                            key={key}
                  ...
                  <div className="space-y-4">
                    ...
                    {/* Vista previa */}
                    {preview.length > 0 && (() => {
                      const sisTotal     = preview.reduce((s, r: any) => s + (r.employerContributions?.sis ?? 0), 0);
                      const afcEmpTotal  = preview.reduce((s, r: any) => s + (r.employerContributions?.afcEmployer ?? 0), 0);
                      const cai111Total  = preview.reduce((s, r: any) => s + (r.employerContributions?.cai111 ?? 0), 0);
                      const mutualTotal  = preview.reduce((s, r: any) => s + (r.employerContributions?.mutual ?? 0), 0);
                      const ccafTotal    = preview.reduce((s, r: any) => s + (r.employerContributions?.ccaf ?? 0), 0);
                      const totalEmpCost = preview.reduce((s, r: any) => s + (r.totalEmployerCost ?? 0), 0);
                      const totalNet     = preview.reduce((s, r: any) => s + (r.netPay ?? 0), 0);
                      const aportesTotal = sisTotal + afcEmpTotal + cai111Total + mutualTotal + ccafTotal;
                      return (
                        <>
                      <div className="rounded-xl bg-blue-50 border border-blue-200 p-4 space-y-2">
                        ...
                        {preview.some(r => r.warnings.length > 0) && (
                          <div className="mt-2 rounded-lg bg-amber-50 border border-amber-200 p-3 space-y-2">
                            <div className="flex items-center gap-1.5">
                              <AlertTriangle className="w-3.5 h-3.5 text-amber-600 shrink-0" />
                              <p className="text-[11px] font-semibold text-amber-800">
                                {preview.flatMap(r => r.warnings).length} advertencia(s)
                              </p>
                            </div>
                            <ul className="space-y-1.5 pl-5">
                              {preview.flatMap(r =>
                                r.warnings.map((w: string, idx: number) => ({
                                  workerName: r.workerName, msg: w, key: `${r.workerId}-${idx}`,
                                }))
                              ).map(item => (
                                <li key={item.key} className="text-[10.5px] text-amber-900 leading-snug list-disc">
                                  <span className="font-semibold">{item.workerName}:</span> {item.msg}
                                </li>
                              ))}
                            </ul>
                          </div>
                      ...
                      {/* Aportes empleador (costos adicionales al líquido pagado) */}
                      <div className="rounded-xl bg-zinc-50 border border-zinc-200 p-4 space-y-2">
                        <p className="text-xs font-semibold text-zinc-700">Aportes empleador (costo adicional)</p>
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-[11px]">
                          {sisTotal > 0 && (
                            <div className="flex justify-between bg-white rounded-lg px-2 py-1.5">
                              <span className="text-zinc-500">SIS</span>
                              <span className="font-medium text-zinc-800">${fmt(sisTotal)}</span>
                            </div>
                          )}
                          {afcEmpTotal > 0 && (
                            <div className="flex justify-between bg-white rounded-lg px-2 py-1.5">
                              <span className="text-zinc-500">AFC empleador</span>
                              <span className="font-medium text-zinc-800">${fmt(afcEmpTotal)}</span>
                            </div>
                          )}
                          {cai111Total > 0 && (
                            <div className="flex justify-between bg-white rounded-lg px-2 py-1.5">
                              <span className="text-zinc-500">CAI 1.11%</span>
                              <span className="font-medium text-zinc-800">${fmt(cai111Total)}</span>
                            </div>
                          )}
                          {mutualTotal > 0 && (
                            <div className="flex justify-between bg-white rounded-lg px-2 py-1.5">
                              <span className="text-zinc-500">Mutual</span>
                              <span className="font-medium text-zinc-800">${fmt(mutualTotal)}</span>
                            </div>
                          )}
                          {ccafTotal > 0 && (
                            <div className="flex justify-between bg-white rounded-lg px-2 py-1.5">
                              <span className="text-zinc-500">CCAF 0.6%</span>
                              <span className="font-medium text-zinc-800">${fmt(ccafTotal)}</span>
                            </div>
                          )}
                          <div className="flex justify-between bg-zinc-200 rounded-lg px-2 py-1.5">
                            <span className="text-zinc-700 font-semibold">Total aportes</span>
                            <span className="font-bold text-zinc-900">${fmt(aportesTotal)}</span>
                          </div>
                        </div>
                        <div className="flex justify-between pt-2 border-t border-zinc-200 text-xs">
                          <span className="text-zinc-600">Costo total empleador este mes</span>
                          <span className="font-bold text-zinc-900">${fmt(totalEmpCost)}</span>
                        </div>
                        <p className="text-[10px] text-zinc-400 leading-tight">
                          Líquido a pagar: ${fmt(totalNet)} · Aportes adicionales no van al trabajador.
                        </p>
                      </div>
                      </>
                    );
                    ...
                    {blockClose && (
                      <div className="rounded-xl bg-amber-50 border border-amber-200 p-3 flex items-start gap-2">
                        ...
                        <p className="text-xs text-amber-800 leading-snug">
                          No puedes cerrar este mes hasta que cierres <strong>{periodoLabel(prevPeriod)}</strong>.
                          Los períodos se deben cerrar en orden cronológico.
                        </p>
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between pt-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          disabled={isProc || blockClose}
                          onClick={() => handleProcesar(period, 'final')}
                          title={blockClose ? `Debes cerrar primero ${periodoLabel(prevPeriod)}` : ''}
                          className="text-sm px-5 py-2 rounded-lg bg-[#1a2e6e] text-white font-medium hover:bg-[#142358] transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                        >
                          {isProc
      ...
      {liquidacionesModal && (
        <LiquidacionesModal
          period={liquidacionesModal}
          onClose={() => setLiquidacionesModal(null)}
        />
      )}
    </div>
  );
