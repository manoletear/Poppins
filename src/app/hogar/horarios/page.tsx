...
export default function HorariosPage() {
  ...
  useEffect(() => {
    // Fuente local Poppins (reemplaza /api/buk/*). Si falla, queda silencioso.
    fetch('/api/empresa/vacaciones-historico').then((r) => r.json()).then((d) => { if (d?.ok) setVacEmpleados(d.items || []); }).catch(() => {});
    fetch('/api/empresa/liquidaciones-historico').then((r) => r.json()).then((d) => { if (d?.ok) setLiqEmpleados(d.items || []); }).catch(() => {});
  }, []);
