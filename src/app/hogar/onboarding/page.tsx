...
import { validateRut, cleanRut } from '@/lib/validators';
...
export default function OnboardingPage() {
  ...
  async function handleContratanteNext() {
    ...
    if (!validateRut(rut)) {
      setError('El RUT del empleador es inválido. Revisa el dígito verificador (ej. 12.345.678-9).');
      return;
    }
    // Normalizar RUT antes de guardar (sin puntos, con guión)
    const rutNorm = cleanRut(rut);
    const rutFmt = rutNorm.length > 1 ? `${rutNorm.slice(0, -1)}-${rutNorm.slice(-1)}` : rutNorm;
    setContratante({ ...contratante, rut: rutFmt });
    setError(null);
    setStep(2);
  }
  ...
  async function handleElegirPlan(plan: PlanTipo, ciclo: CicloFacturacion) {
    ...
    try {
      ...
      if (plan !== 'starter') {
        ...
        // crea al volver (/hogar/suscripcion/confirmar). Simulado: ya quedó creada.
